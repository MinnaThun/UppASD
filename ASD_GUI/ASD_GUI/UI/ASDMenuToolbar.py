...
def VTK_Menu_and_Toolbar_Setup(window):
    ...
    window.TifeEneButton.toggled.connect(window.set_energy_proj)
...
def Plot_Menu_and_Toolbar_Setup(window):
    ...
    window.EneTifeCheck.toggled.connect(window.PlotEneCompSelector)
...
def Input_Toolbar_Setup(window):
    ...
    window.InpTifeCheck.toggled.connect(window.getInpFile)
